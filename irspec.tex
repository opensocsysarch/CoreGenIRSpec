...
\pagestyle{fancy}
\lhead{}
\chead{\textbf{CoreGen IR Spec v.1.5}}
\rhead{}
\lfoot{} %-- format: TR YYYY-RRR-V.V; y = year; r = report; v = version
\cfoot{\textbf{CoreGen IR Spec v.1.5}}
...
\begin{center}
\begin{tabular}{l r}
Date: & March 31, 2022 \\ % Date the experiment was performed
Revision: & 1.5 \\         % revision number
...
\begin{versionhistory}
  ...
  \vhEntry{1.4.5}{04.28.2020}{JLeidel}{Adding LineSize cache line size parameter and memory ordering parameters; Adding read and write ports to register classes; Adding DataPath nodes; Adding RegIsDestination keyword to instruction formats}
  \vhEntry{1.4.6}{06.03.2020}{JLeidel}{Adding scheduler parameters to core nodes; Adding virtual to physical controllers to extensions}
  \vhEntry{1.5}{03.31.2022}{JLeidel}{Adding vector and matrix register syntax; Adding VLIWStage node definitions}
\end{versionhistory}
...
\item \textbf{VLIWStage}: Explicitly wired stages for a VLIW pipeline with explicit input \& output connectivity.
...
\hline
\multirow{2}{*}{\texttt{Vector}} & Sets \texttt{vector} parameter for the target register.\\
						& Requires that the user initialize the \texttt{DimX} parameter. (Bool)\\
\hline
\multirow{2}{*}{\texttt{Matrix}} & Sets \texttt{vector} parameter for the target register.\\
						& Requires that the user initialize the \texttt{DimX} and \texttt{DimY} parameters. (Bool)\\
\hline
\multirow{2}{*}{\texttt{DimX}} & Sets the number of (\texttt{Width}) elements in the \texttt{X} dimension for a vector or matrix register\\
						& (Unsigned Int)\\
\hline
\multirow{2}{*}{\texttt{DimY}} & Sets the number of (\texttt{Width}) elements in the \texttt{Y} dimension for a matrix register\\
						& (Unsigned Int)\\
\hline
...
\clearpage
\subsection{VLIW Stages}
\label{sec:VLIWStageNodes}

VLIWStage nodes are utilized to represent a single stage in an explicit VLIW pipeline.  This is an \textbf{advanced} 
feature of the CoreGen infrastructure and IR that allows architects to explicit define the inputs, outputs and 
connectivity between VLIW-style pipeline stages.  VLIW pipeline stages differ in form from 
traditional instruction (\texttt{Inst}) blocks in that it requires that the user explicitly outline the set of input and output 
I/O's between pipeline stages.  Further, the naming convention for VLIW pipeline stages must follow the 
form : \texttt{NAME\_STAGE}, where \texttt{NAME} is a unique stage name and \texttt{STAGE } is a monotonically 
increasing integer value.  The CoreGen infrastructure automatically wires the various pipeline stages together 
in the right order using the monotonically increasing integers.  This also includes the ability to wire multiple, disparate 
pipeline stages that exist at the same clock cycle.  Examples of utilizing this naming convention is noted as follows:

\begin{verbatim}
# Example 1: Simple VLIW pipeline
STAGE_0
STAGE_1
STAGE_2
...

# Example 2: Complex VLIW pipeline
STAGE_0     # clock 0
STAGEA_1    # clock 1, pipe A
STAGEB_1    # clock 1, pipe B
STAGE_2     # clock 2
\end{verbatim}

The VLIWStage nodes a require several fields and have several optional fields (Listing~\ref{lis:vliwstage}).  First, 
the \texttt{VLIWStageName} must be specified in the form mentioned above.  We must also associate the stage 
with a valid instruction format (\texttt{InstFormat}).  Ideally, the target instruction format is defined as a VLIW format.  
Next, we specify the set of I/O operations as \texttt{Inputs} and \texttt{Outputs}.  These must be present, but may 
not have any actual input or output values.  For example, the first stage of the pipeline may not have any inputs 
and the last stage may not have any outputs.  However, for any stage that has an input, there must be an equivalent 
output from a previous pipeline stage.  Finally, there is an optional \texttt{Impl} field whereby users can 
specify the functional body of the stage, much in the same way as the instruction node definition.  

\vspace{0.125in}
\begin{lstlisting}[frame=single,style=base,caption={VLIWStage Node Definition},captionpos=b,label={lis:vliwstage}]
VLIWStages:
  - VLIWStageName: @String@
    InstFormat: @String@
    Inputs:
      - @String@
    Outputs:
...
An example of creating VLIWStage nodes is noted as follows:

\vspace{0.125in}
\begin{lstlisting}[frame=single,style=base,caption={VLIWStage Node Example},captionpos=b,label={lis:vliwstageex}]
VLIWStages:
  - VLIWStageName: STAGE_0
    InstFormat: SAMPLE.VLIW
    Inputs:
      []
    Outputs:
      - RS0_VAL
      - RS1_VAL
    Impl: |
      u64 RS0_VAL = LOADELEM(RS0, 64)
      u64 RS1_VAL = LOADELEM(RS0, 64)
  - VLIWStageName: STAGE_1
    InstFormat: SAMPLE.VLIW
    Inputs:
      - RS0_VAL
      - RS1_VAL
    Outputs:
      - RD_VAL
    Impl: |
      u64 RD_VAL = RS0_VAL + RS1_VAL
  - VLIWStageName: STAGE_2
    InstFormat: SAMPLE.VLIW
    Inputs:
      - RD_VAL
    Outputs:
...
\item Virtual to Physical Controllers
